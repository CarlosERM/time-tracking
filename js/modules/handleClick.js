export default function handleClick() {
  ...
  async function changeText(element) {
    ...
    data.forEach((category) => {
      // percorre o objeto.
      const name = category.title.toLowerCase().replace(/ /g, "");
      // pega os title desse objeto. ex: work, play, study...
      const current = document.querySelector(`.${name} .current`); // com esses title é possível selecioanr os current e os previous correspondentes a essas categorias.
      const previous = document.querySelector(`.${name} .previous`);

      const time = category.timeframes[type]; // aqui são separados as horas correspondentes a cada categoria.
      ...
      else previous.innerHTML = `Last Month - ${time.previous}hrs`;
    });
  }
